...
--Giá bán của một sản phẩm được nhập bằng 160% giá nhập sản phẩm đó
CREATE OR REPLACE TRIGGER GIABAN_SANPHAM 
AFTER INSERT OR UPDATE ON CTPN
FOR EACH ROW
DECLARE
BEGIN
    UPDATE SANPHAM
    SET GIA = :NEW.GIANHAP * 1.6
    WHERE MASP = :NEW.MASP;
END;
/
...
CREATE OR REPLACE FUNCTION INSERT_PHIEUNHAP(var_mancc PHIEUNHAP.MANCC%TYPE, var_manv PHIEUNHAP.MANV%TYPE) 
RETURN NUMBER AS
BEGIN
    INSERT INTO PHIEUNHAP(MAPHIEUNHAP, MANCC, NGAYNHAP, MANV)
    VALUES(SEQ3_MAPHIEUNHAP.NEXTVAL,var_mancc, SYSDATE ,var_manv);
    COMMIT;
    RETURN 1;
EXCEPTION
    WHEN OTHERS THEN
...
END;
/
CREATE OR REPLACE PROCEDURE GET_SANPHAM_SOLUONG(var_bien in CTHD.MASP%TYPE,  cursor OUT SYS_REFCURSOR)
AS
BEGIN
    OPEN cursor FOR
    SELECT * FROM CTHD SP WHERE MASP = var_bien;
    COMMIT;
END;
/
CREATE OR REPLACE PROCEDURE GET_SANPHAM_HOADON(var_bien in CTHD.SOHD%TYPE,  cursor OUT SYS_REFCURSOR)
AS
BEGIN
    OPEN cursor FOR
    SELECT * FROM SANPHAM SP, CTHD WHERE SP.MASP = CTHD.MASP AND SOHD = var_bien ORDER BY SOHD;
    COMMIT;
END;
/
...
BEGIN
    OPEN cursor FOR
    SELECT distinct(s.MASP), s.* FROM SANPHAM s, CTPN 
...
END;
/
CREATE OR REPLACE PROCEDURE GET_KHUYENMAI_HOADON(var_bien in CTKM.SOHD%TYPE,  cursor OUT SYS_REFCURSOR)
AS
BEGIN
    OPEN cursor FOR
    SELECT * FROM KHUYENMAI KM, CTKM WHERE KM.MAKM = CTKM.MAKM AND SOHD = var_bien ORDER BY SOHD;
    COMMIT;
END;
/
--BẢNG NHÀ CUNG CẤP
...
BEGIN
    OPEN cursor FOR
    SELECT * FROM HOADON WHERE TRIGIAHD != 0 ORDER BY SOHD;
    COMMIT;
END;
/
...
BEGIN
    OPEN cursor FOR
    SELECT MAPHIEUNHAP,MANV, MANCC, NGAYNHAP, TONGTIENNHAP FROM PHIEUNHAP
    WHERE TONGTIENNHAP != 0 ORDER BY MAPHIEUNHAP;
    COMMIT;
END;
/
CREATE OR REPLACE PROCEDURE GET_CTPN_ALL(var_bien in CTPN.MAPHIEUNHAP%TYPE, cursor OUT SYS_REFCURSOR)
AS
BEGIN
    OPEN cursor FOR
    SELECT * FROM CTPN WHERE MAPHIEUNHAP = var_bien ORDER BY MAPHIEUNHAP;
    COMMIT;
END;
/
...
BEGIN
    OPEN cursor FOR
    SELECT * FROM PHIEUNHAP
    WHERE  MAPHIEUNHAP = var_bien AND TONGTIENNHAP != 0 ORDER BY MAPHIEUNHAP;
    COMMIT;
END;
/
...
BEGIN
    ...
    WHERE  MANV = var_bien AND TONGTIENNHAP != 0 ORDER BY MAPHIEUNHAP;
    COMMIT;
END;
/
